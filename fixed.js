window.addEventListener("DOMContentLoaded", () => {
  ...
  const layerMeasurementsAccordionBtn = document.getElementById("layerMeasurementsAccordionBtn");
  const layerMeasurementsDetails = document.getElementById("layerMeasurementsDetails");
  const layerMeasurementItemList = document.getElementById("layerMeasurementItemList");
  const layerMeasurementsCount = document.getElementById("layerMeasurementsCount");
  ...
  function applyMeasurementLayerFilter() {
    measurements.forEach(item => {
      if (!item || !item.layer) return;
      const visible = layerVisibility.measurements !== false && item.visible !== false;
      if (visible) {
        if (!measureLayer.hasLayer(item.layer)) measureLayer.addLayer(item.layer);
        if (item.labelMarker && !measureLayer.hasLayer(item.labelMarker)) measureLayer.addLayer(item.labelMarker);
      } else {
        if (measureLayer.hasLayer(item.layer)) measureLayer.removeLayer(item.layer);
        if (item.labelMarker && measureLayer.hasLayer(item.labelMarker)) measureLayer.removeLayer(item.labelMarker);
      }
    });
    if (layerMeasurementsCount) layerMeasurementsCount.textContent = String(measurements.length);
    renderLayerMeasurementItemList();
  }

  function renderLayerMeasurementItemList() {
    if (!layerMeasurementItemList) return;
    layerMeasurementItemList.innerHTML = "";
    if (!measurements.length) {
      layerMeasurementItemList.innerHTML = '<p class="layerDynamicEmpty">面積図形はありません。</p>';
      return;
    }
    measurements.forEach(item => {
      const label = document.createElement("label");
      label.className = "layerSubToggle layerMeasurementToggle";
      ...
      dot.className = "layerMeasurementColor";
      dot.style.background = item.style?.lineColor || "#7e22ce";
      const name = document.createElement("span");
      name.className = "layerMeasurementName";
      const displayName = getMeasurementDisplayName(item);
      name.textContent = `${item.number} ${displayName}　${formatNumber(item.areaHa, 3)}ha`;
      name.title = `${item.number} ${displayName} / ${formatNumber(item.areaM2)}㎡ / ${formatNumber(item.areaHa, 3)}ha`;
      label.append(input, dot, name);
      layerMeasurementItemList.appendChild(label);
    });
  }
  ...
  function setupLayerControls() {
    ...
    if (layerMeasurementsAccordionBtn) layerMeasurementsAccordionBtn.addEventListener("click", () => setLayerDetailsExpanded(layerMeasurementsDetails, layerMeasurementsAccordionBtn, layerMeasurementsAccordionBtn.getAttribute("aria-expanded") !== "true", "計測"));
    ...
    if (showAllLayersBtn) {
      showAllLayersBtn.addEventListener("click", () => {
        ...
        measurements.forEach(item => { item.visible = true; });
  ...
  function setupDirectProjectSaveButton() {
    // Build026.1: プロジェクト保存は保存センターの「.glink保存」に統一する。
    // 復元エンジンと saveGlinkFile 本体は保守用に残すが、画面上の直接保存ボタンは追加しない。
    return;
  }
  ...
  function cloneLatLngPoints(points) {
    return (points || []).map(point => L.latLng(Number(point.lat), Number(point.lng)));
  }
  ...
  function applyMeasurementVertexPositions(measurement, markers, renderList = false) {
    if (!measurement?.layer || !Array.isArray(markers) || markers.length < 3) return;
    const openPoints = markers.map(marker => marker.getLatLng());
    measurement.layer.setLatLngs(openPoints);
    refreshMeasurementStats(measurement);
    updateMeasureSummaryBanner();
    if (renderList) renderMeasureList();
    applyMeasurementLayerFilter();
  }
  ...
  function saveMeasurementVertexEdit() {
    if (!measurementVertexEditState) return;
  ...
  function deleteMeasurement(id) {
    ...
    if (measurementVertexEditState?.measurement === target) {
      clearMeasurementVertexHandles();
      measurementVertexEditState = null;
      setMeasurementVertexActionState(false);
    }
  ...
  function normalizeTextFontFamily(value) {
    if (value === "serif") return '"Yu Mincho","Hiragino Mincho ProN",serif';
    if (value === "rounded") return '"Hiragino Maru Gothic ProN","Yu Gothic",sans-serif';
  ...
  function getNormalizedTextVisuals(data) {
    ...
    return {
      ...
      height: Math.max(30, Math.min(600, Number(data.boxHeight || 80))),
      fontSize: Math.max(12, Math.min(96, Number(data.fontSize || 28))),
      fontWeight: data.bold === false ? "400" : "700",
      color: data.color || "#e60000",
      fontFamily: normalizeTextFontFamily(data.fontFamily)
    };
  }

  function createTextIcon(data, selected = false) {
    const visual = getNormalizedTextVisuals(data);
    const writing = visual.orientation === "vertical"
    ...
    const textStyle = `color:${escapeTextHtml(visual.color)};font-family:${visual.fontFamily};font-size:var(--map-text-font-size)!important;font-weight:${visual.fontWeight};${writing}`;
    const html = `<div class="mapTextLabel${selectedClass}" style="--map-text-font-size:${visual.fontSize}px;background:${escapeTextHtml(visual.background)};border:${visual.border};width:${visual.width}px;height:${visual.height}px;"><span class="mapTextContent" style="${textStyle}">${escapeTextHtml(data.text).replace(/\n/g,'<br>')}</span>${handles}</div>`;
    return L.divIcon({
      className: "mapTextIcon",
      html,
      iconSize: [visual.width, visual.height],
      iconAnchor: [visual.width / 2, visual.height / 2]
    });
  }

  function applyTextStylesToRenderedElement(item) {
    const markerElement = item?.marker?.getElement?.();
    if (!markerElement || !item?.data) return false;
    const labelElement = markerElement.querySelector(".mapTextLabel");
    const contentElement = markerElement.querySelector(".mapTextContent");
    if (!labelElement || !contentElement) return false;

    const visual = getNormalizedTextVisuals(item.data);
    labelElement.style.setProperty("--map-text-font-size", `${visual.fontSize}px`);
    labelElement.style.width = `${visual.width}px`;
    labelElement.style.height = `${visual.height}px`;
    labelElement.style.background = visual.background;
    labelElement.style.border = visual.border;

    contentElement.style.setProperty("font-size", `${visual.fontSize}px`, "important");
    contentElement.style.setProperty("line-height", "1.25", "important");
    contentElement.style.color = visual.color;
    contentElement.style.fontFamily = visual.fontFamily;
    contentElement.style.fontWeight = visual.fontWeight;
    contentElement.style.writingMode = visual.orientation === "vertical" ? "vertical-rl" : "horizontal-tb";
    contentElement.style.textOrientation = visual.orientation === "vertical" ? "upright" : "mixed";
    contentElement.style.whiteSpace = "pre-wrap";

    // ブラウザまたは既存CSSの影響で値が上書きされた場合も、実測値を確認して再適用する。
    const computedSize = Number.parseFloat(window.getComputedStyle(contentElement).fontSize);
    if (!Number.isFinite(computedSize) || Math.abs(computedSize - visual.fontSize) > 0.1) {
      labelElement.style.setProperty("font-size", `${visual.fontSize}px`, "important");
      contentElement.style.setProperty("font-size", `${visual.fontSize}px`, "important");
    }
    contentElement.dataset.appliedFontSize = String(visual.fontSize);
    return true;
  }
  ...
  function refreshTextMarker(item, selected = selectedTextItem === item) {
    ...
    requestAnimationFrame(() => {
      applyTextStylesToRenderedElement(item);
      bindTextResizeHandles(item);
    });
  }
  ...
  function applyTextEditPreview() {
    ...
    // range操作中も、再生成後の実DOMへ直接書式を反映する。
    requestAnimationFrame(() => applyTextStylesToRenderedElement(selectedTextItem));
  }
  ...
  [textEditContent, textEditColor, textEditBackgroundColor, textEditBackgroundOpacity, textEditFontSize, textEditBoxWidth, textEditBoxHeight]
    ...
    .forEach(control => {
      ...
      control.addEventListener("change", handleTextEditControlChange);
    });

  [textEditBackgroundEnabled, textEditFontFamily, textEditBold, textEditOrientation]
    .filter(Boolean)
    .forEach(control => control.addEventListener("change", handleTextEditControlChange));
  ...
  if (saveMeasureVertexEdit) {
    saveMeasureVertexEdit.addEventListener("click", () => {
      saveMeasurementVertexEdit();
    });
  }

  if (cancelMeasureVertexEdit) {
    cancelMeasureVertexEdit.addEventListener("click", () => {
      cancelMeasurementVertexEdit(true);
    });
  }
  ...
  if (saveMeasureEdit) {
    saveMeasureEdit.addEventListener("click", () => {
      if (!selectedMeasurement) return;
      if (measurementVertexEditState?.measurement === selectedMeasurement) saveMeasurementVertexEdit();
  ...
  if (deleteMeasureEdit) {
    deleteMeasureEdit.addEventListener("click", () => {
      ...
      if (!result) return;
      if (measurementVertexEditState?.measurement === selectedMeasurement) {
        clearMeasurementVertexHandles();
        measurementVertexEditState = null;
        setMeasurementVertexActionState(false);
      }
  ...
  function showGlinkObjectDiagnosisPanel() {
    // Build026.1: 開発用診断パネルは完成版UIでは表示しない。
    // 診断データ取得関数は保守用に残す。
    const diagnosis = getGlinkObjectDiagnosis();
    glinkDiagLog("fixed object diagnosis manual hidden", diagnosis);
    return diagnosis;
  }
  ...
  function loadGlinkData(data, options = {}) {
    ...
    runSection("texts", () => { restoreTexts(Array.isArray(data.texts) ? data.texts : []); });
    ...
    if (sectionErrors.length) {
      ...
      if (!options.silent) alert("一部の復元に失敗しました。\n" + msg);
  ...
  function showGlinkRestoreVerificationPanel(saved, restored, reason) {
    // Build026.1: 復元確認パネルは完成版UIでは表示しない。
    // 復元件数は診断ログにのみ保持し、復元エンジン本体は変更しない。
    const ok = ["pins", "drawings", "measurements", "tracks", "activityHistory"].every(key => Number(saved?.[key] || 0) === Number(restored?.[key] || 0));
    glinkDiagLog("fixed restore verification hidden", { saved, restored, reason, ok });
  }

  let glinkStartupRestoreCompleteApplied = false;
  function readGlinkRestoreDataForComplete() {
    if (startupGlinkRestoreData && startupGlinkRestoreData.format === "glink") return startupGlinkRestoreData;
    try {
      const raw = sessionStorage.getItem("gLink_pendingRestoreData") || localStorage.getItem("gLink_pendingRestoreData");
      if (!raw) return null;
      const data = JSON.parse(raw);
      return data && data.format === "glink" ? data : null;
    } catch (error) {
      glinkDiagLog("fixed complete restore read failed", { message: error?.message || String(error) });
      return null;
    }
  }
  ...
  function applyGlinkStartupRestoreComplete(reason = "manual") {
    ...
    glinkStartupRestoreCompleteApplied = true;
    const savedCounts = getGlinkProjectCountSummary(data);
    glinkDiagLog("fixed complete restore start", { reason, savedCounts, summary: glinkDiagSummarizeData(data) });
    try {
      loadGlinkData(data, { silent: true, completeRestore: true, reason });
      const restoredCounts = getCurrentRestoredCountSummary();
      glinkDiagLog("fixed complete restore finished", { reason, savedCounts, restoredCounts, storage: glinkDiagStorageSnapshot() });
      showGlinkRestoreVerificationPanel(savedCounts, restoredCounts, reason);
      try {
        ...
        glinkDiagLog("fixed complete restore storage cleanup warning", { message: storageError?.message || String(storageError) });
      }
      return true;
    } catch (error) {
      glinkStartupRestoreCompleteApplied = false;
      console.error(".glink完全復元に失敗しました。", error);
      glinkDiagLog("fixed complete restore failed", { reason, message: error?.message || String(error), savedCounts });
      alert(".glinkの完全復元に失敗しました。");
      return false;
    }
  }
  ...
  if (isGlinkRestoreMode) {
    ...
    window.setTimeout(() => applyGlinkStartupRestoreComplete("post-init-800ms"), 800);
    try { map.whenReady(() => window.setTimeout(() => applyGlinkStartupRestoreComplete("map-ready"), 120)); } catch (error) {}
  } else if (!restoreWorkingDataAfterSaveCenter()) {
