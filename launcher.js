...
/* Build026.3.5: reliable important-notification queue and re-display control */
(()=>{
'use strict';
if(window.__GLINK_STANDARD_NOTIFICATIONS_LOADED__) return;
window.__GLINK_STANDARD_NOTIFICATIONS_LOADED__=true;
console.info('[G-Link Notify]',{time:new Date().toISOString(),build:'Build026.3.5',event:'S026350-SCRIPT-LOADED'});
const BUILD='Build026.3.5',PORTAL_BASE='https://g-link-portal.pages.dev',LOCAL_KEY='gLink_standardAuthRemembered',SESSION_KEY='gLink_standardAuthSession';
...
function create(){if(root)return;const stage=document.getElementById('launcherStage');if(!stage)return;let rail=stage.querySelector('#glinkStatusRail');if(!rail){rail=document.createElement('div');rail.id='glinkStatusRail';rail.className='glink-status-rail';stage.appendChild(rail);}const licenseButton=document.createElement('button');licenseButton.id='glinkLicenseBadge';licenseButton.className='glink-status-icon license-badge';licenseButton.type='button';licenseButton.style.order='2';licenseButton.hidden=true;licenseButton.innerHTML='<span aria-hidden="true">●</span>';const bellButton=document.createElement('button');bellButton.id='glinkNoticeBell';bellButton.className='glink-status-icon notice-bell';bellButton.type='button';bellButton.style.order='3';bellButton.setAttribute('aria-label','お知らせ');bellButton.title='お知らせ';bellButton.innerHTML='<span aria-hidden="true">🔔</span><b id="glinkNoticeCount" hidden>0</b>';rail.append(licenseButton,bellButton);root=document.createElement('div');root.id='glinkNoticeCenter';root.innerHTML=`<section id="glinkNoticePanel" class="notice-panel" hidden><header><div><strong>お知らせ</strong><small id="glinkNoticeStatus">取得中…</small></div><button id="glinkNoticeClose" type="button">×</button></header><div id="glinkNoticeList" class="notice-list"></div><footer><button id="glinkReadAll" type="button">すべて既読</button></footer></section><div id="glinkImportantOverlay" class="important-overlay" hidden><article><span class="important-label">重要なお知らせ</span><h2 id="glinkImportantTitle"></h2><p id="glinkImportantBody"></p><a id="glinkImportantLink" target="_blank" rel="noopener" hidden>詳細を見る</a><button id="glinkImportantClose" type="button">確認しました</button></article></div>`;stage.appendChild(root);bellButton.addEventListener('click',()=>{const p=root.querySelector('#glinkNoticePanel');p.hidden=!p.hidden;if(!p.hidden)markVisibleRead();});licenseButton.addEventListener('click',()=>{if(state.license){const d=state.license.daysRemaining;licenseButton.title=d<0?'ライセンス期限切れ':d===0?'ライセンスは本日期限です':`ライセンス残り${d}日`;}});root.querySelector('#glinkNoticeClose').addEventListener('click',()=>root.querySelector('#glinkNoticePanel').hidden=true);root.querySelector('#glinkReadAll').addEventListener('click',()=>{hideImportant('read-all');markRead(state.items.filter(x=>!x.read).map(x=>x.key));});root.querySelector('#glinkImportantClose').addEventListener('click',()=>{const key=root.querySelector('#glinkImportantOverlay').dataset.key;hideImportant('confirmed');diag('S026340-IMPORTANT-CONFIRMED',{key});markRead([key]);});root.querySelector('#glinkNoticeList').addEventListener('click',e=>{const card=e.target.closest('[data-notice-key]');if(card)markRead([card.dataset.noticeKey]);});}
...
function renderLicense(){const b=document.getElementById('glinkLicenseBadge'),d=state.license?.daysRemaining;if(!b)return;if(d===null||d===undefined){b.hidden=true;return;}b.hidden=false;b.className='glink-status-icon license-badge '+(d<=7?'danger':d<=30?'warning':'normal');b.innerHTML='<span aria-hidden="true">●</span>';const label=d<0?'ライセンス期限切れ':d===0?'ライセンスは本日期限です':`ライセンス残り${d}日`;b.title=label;b.setAttribute('aria-label',label);diag('S026301-LICENSE',{daysRemaining:d});}
