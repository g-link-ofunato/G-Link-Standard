(() => {
  ...
  async function login(event){event.preventDefault();const button=gate.querySelector('#glinkAuthSubmit');button.disabled=true;message('認証情報を確認しています…',true);try{const licenseId=gate.querySelector('#glinkLicenseId').value.trim().toUpperCase();const password=gate.querySelector('#glinkLicensePassword').value;const remember=gate.querySelector('#glinkRemember').checked;const location=await captureLocationData();const response=await api('/api/app/auth/login',{method:'POST',body:JSON.stringify({licenseId,password,remember,location})});const data=await parseResponse(response);if(data.mustChange){diag('P001421-FIRSTLOGIN',{licenseId});const params=new URLSearchParams({setupToken:data.setupToken,remember:remember?'1':'0'});location.href=`${data.passwordChangeUrl}#${params.toString()}`;return;}const state={token:data.token,expiresAt:data.expiresAt,lastValidatedAt:new Date().toISOString(),organization:data.organization,remember};saveState(state,remember);activate(state,false);}catch(error){message(`${error.message}${error.data?.diagnosticCode?'（'+error.data.diagnosticCode+'）':''}`);}finally{button.disabled=false;}}
